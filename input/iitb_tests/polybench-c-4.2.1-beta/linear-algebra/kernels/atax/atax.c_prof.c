...
static void kernel_atax(int m, int n, double A[4000 + 0][4000 + 0], double x[4000 + 0], double y[4000 + 0], double tmp[4000 + 0])
{
  ...
  double beta = 0.0;
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, alpha, A, n, x, 1, beta, z, 1);
  clock_gettime(CLOCK_MONOTONIC_RAW, &end);
  cpu_time_used = (((double) end.tv_sec) + (1.0e-9 * end.tv_nsec)) - (((double) start.tv_sec) + (1.0e-9 * start.tv_nsec));
  printf("{\"In_Function\":\"kernel_atax\",\"Block_Type\":\"FuncCall\",\"Block_No\":\"8\",\"Function_Name\":\"cblas_dgemv\",\"Time\":\"%.10f\"}\n", cpu_time_used);
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
  cblas_dgemv(CblasRowMajor, CblasTrans, n, n, alpha, A, n, z, 1, beta, y, 1);
  clock_gettime(CLOCK_MONOTONIC_RAW, &end);
  cpu_time_used = (((double) end.tv_sec) + (1.0e-9 * end.tv_nsec)) - (((double) start.tv_sec) + (1.0e-9 * start.tv_nsec));
  printf("{\"In_Function\":\"kernel_atax\",\"Block_Type\":\"FuncCall\",\"Block_No\":\"12\",\"Function_Name\":\"cblas_dgemv\",\"Time\":\"%.10f\"}\n", cpu_time_used);
}
